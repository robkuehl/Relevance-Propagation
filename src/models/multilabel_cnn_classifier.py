...
class cnn_classifier:
    ...
    def __init__(self, model_type:str, dataset:str):
        ...
        self.train_images, self.test_images, self.train_labels, self.test_labels = get_training_data(dataset=dataset, test_size=0.2)

        # Encode labels
        onehot_encoder = OneHotEncoder(sparse=False)
        self.train_labels_enc = onehot_encoder.fit_transform(self.train_labels)
        self.test_labels_enc = onehot_encoder.fit_transform(self.test_labels)
    ...
    def run_model(self, batch_size, epochs):
        ...
        self.model.fit(x=self.train_images, y=self.train_labels_enc, batch_size=batch_size, epochs=epochs, callbacks=[checkpoint,early], validation_split=0.1)
        self.model.evaluate(x=self.test_images, y=self.test_labels_enc)
